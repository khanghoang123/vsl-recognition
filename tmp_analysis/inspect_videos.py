...
VIDEOS = [
    Path("/mnt/c/Users/Acer Nitro/Downloads/992931.mp4"),
    Path("/mnt/c/Users/Acer Nitro/Downloads/7842195253917.mp4"),
]
OUT_DIR = Path("/mnt/d/Documents/hoc_ky_8/Thi_giac_may_tinh/Cuoi_ky/vsl-recognition/tmp_analysis")
...
def main():
    ...
    for video_path in VIDEOS:
        ...
        if not cap.isOpened():
            report[video_path.name] = {"error": "cannot_open"}
            continue
        ...
        positions = np.linspace(0, max(frame_count - 1, 0), 6, dtype=int) if frame_count else np.array([], dtype=int)
        frames = []
        brightness = []
        blur = []
    ...
    with open(OUT_DIR / "video_report.json", "w", encoding="utf-8") as f:
